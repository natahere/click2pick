...
.selbox{background:#13200f;border-left:3px solid #34d399;border-radius:6px;
    padding:9px 13px;font-size:12px;color:#6be09a;margin-bottom:10px}
...
def annotate(img, boxes, sel=None):
    out  = img.copy()
...
DEFAULTS = dict(pages=[], pdf_texts=[], extracted={}, boxes=[],
                ...
                fields=list(PRESETS["📄 Invoice"]), sel=None)
for k, v in DEFAULTS.items():
    if k not in st.session_state:
        st.session_state[k] = v

# ── Read selection from query params (posted by canvas form submit) ───────────
...
<div style="padding:20px 0 14px;border-bottom:1px solid #26262d;margin-bottom:20px">
  ...
  <div style="font-size:12px;color:#6b6b78;margin-top:3px">
    Upload · Drag on image to select region · Click Extract in left panel</div>
</div>""", unsafe_allow_html=True)

left, right = st.columns([1, 1.9], gap="large")

# ══════════════════════════════════════════════════════════════════════════════
# LEFT — controls + extract button lives here, always visible
# ══════════════════════════════════════════════════════════════════════════════
with left:
    ...
    # ── Selection status + Extract button — always in left panel ──────────────
    sel = st.session_state.sel
    if sel and sel["x2"] - sel["x1"] > 8 and sel["y2"] - sel["y1"] > 8:
        # Crop preview
        if st.session_state.pages:
            page_img_l = st.session_state.pages[st.session_state.page_idx]
            iw_l, ih_l = page_img_l.size
            pad  = 6
            crop = page_img_l.crop((max(0, sel["x1"]-pad), max(0, sel["y1"]-pad),
                                     min(iw_l, sel["x2"]+pad), min(ih_l, sel["y2"]+pad)))
            st.markdown('<div class="panel-label">🔎 Selected Region</div>', unsafe_allow_html=True)
            st.image(crop, use_container_width=True)

        st.markdown(f'<div class="selbox">✅ Region ready — click Extract to run OCR</div>',
                    unsafe_allow_html=True)

        lbl = (f"🔍 Extract → \"{st.session_state.active_field}\""
               if st.session_state.active_field else "🔍 Extract Text")
        ecol, rcol = st.columns([3, 1])
        with ecol:
        ...
        with rcol:
            if st.button("↺", use_container_width=True, help="Clear selection"):
                st.session_state.sel = None
                st.rerun()
    else:
        if not st.session_state.active_field:
            st.markdown('<div class="infobox">① Select a Target Field above<br>'
                        '② Drag on the document to select a region<br>'
                        '③ Click Extract here</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="infobox">🖱 Drag on the document to select<br>'
                        f'the region for <strong>{st.session_state.active_field}</strong></div>',
                        unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# RIGHT — document viewer + drag canvas
# ══════════════════════════════════════════════════════════════════════════════
with right:
    ...
    # Render annotated image into canvas
    ann = annotate(page_img, st.session_state.boxes, st.session_state.sel)
    b64 = pil_to_b64(ann.resize((DISP_W, disp_h), Image.LANCZOS))

    import streamlit.components.v1 as stc
    stc.html(f"""<!DOCTYPE html><html><head>
<style>
...
<div id="msg">🖱 Click and drag to select a region — Extract button appears on the left</div>
<form id="f" method="GET" target="_parent">
  <input type="hidden" id="sel_val" name="sel" value="">
</form>
<script>
const canvas=document.getElementById('c'),ctx=canvas.getContext('2d');
const msg=document.getElementById('msg');
const form=document.getElementById('f'),selVal=document.getElementById('sel_val');
...
function redraw(rx,ry,rw,rh){{
  ...
  if(rw>2&&rh>2){{
    ctx.fillStyle='rgba(245,158,11,0.2)';ctx.fillRect(rx,ry,rw,rh);
...
function done(ex,ey){{
  ...
  redraw(Math.min(sx,ex),Math.min(sy,ey),dw,dh);
  if(dw>4&&dh>4){{
    ...
    msg.textContent='✓ Selected — Extract button now visible on the LEFT panel';
    msg.style.color='#34d399';
    selVal.value=x1+','+y1+','+x2+','+y2;
...
canvas.addEventListener('mousedown',e=>{{[sx,sy]=pos(e);dragging=true;e.preventDefault();}});
canvas.addEventListener('mousemove',e=>{{if(!dragging)return;const[ex,ey]=pos(e);redraw(Math.min(sx,ex),Math.min(sy,ey),Math.abs(ex-sx),Math.abs(ey-sy));}});
canvas.addEventListener('mouseup',e=>{{if(!dragging)return;const[ex,ey]=pos(e);done(ex,ey);}});
canvas.addEventListener('touchstart',e=>{{[sx,sy]=pos(e);dragging=true;e.preventDefault();}},{{passive:false}});
canvas.addEventListener('touchmove',e=>{{if(!dragging)return;const[ex,ey]=pos(e);redraw(Math.min(sx,ex),Math.min(sy,ey),Math.abs(ex-sx),Math.abs(ey-sy));e.preventDefault();}},{{passive:false}});
canvas.addEventListener('touchend',e=>{{if(!dragging)return;const[ex,ey]=pos(e);done(ex,ey);}});
</script></body></html>""", height=disp_h+30, scrolling=False)
